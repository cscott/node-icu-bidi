...
var opts = {
    ...
    uri: 'http://node-icu-bidi.s3.amazonaws.com/',
    tool: 'node-gyp',
    paths: {}
}
...
function test(opts,try_build,callback) {
    ...
    cp.execFile(shell_cmd, args, function(err, stdout, stderr) {
        if (err || stderr) {
            ...
            log('Sweet: "' + opts.binary.filename() + '" is valid, node-icu-bidi is now installed!');
            return callback();
        }
    });
}
...
if (opts.force) {
    ...
    try {
        ...
        var tmpdir = path.join(tmpdirbase,'node-icu-bidi-'+opts.binary.configuration);
        try {
            mkdirp.sync(tmpdir);
        } catch (err) {
            log_debug(err);
